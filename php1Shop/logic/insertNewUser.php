...
if (isset($_POST['insert'])) {
    ...
    if (!preg_match($regexName, $name)) {
        array_push($msg, "Name must have start with uppercase letter and has at least 2 characters!");
        $errors++;
    }

    if (!preg_match($regexLastName, $lastname)) {
        array_push($msg, "Lastname must have start with uppercase letter and has at least 2 characters!");
        $errors++;
    }

    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        array_push($msg, "Email is not good!");
        $errors++;
    }
    ...
    if ($roleID == 0) {
        array_push($msg, "You have to choose role <br>");
        $errors++;
    }
    ...
    if ($errors == 0) {
        ...
        try {
            ...
            //mail($email, "Registration Time Zone", "Click the link to verify your registration: http://localhost:8080/php1/sajt/logic/verificationOfRegistration.php?code=" . $code);
